...
class PostgresTransport extends Transport {
   ...
   */
  init() {
    const { sql, tableName, tableFieldsDef } = this;
    const createTable = async (cmd) => {
      console.log(`CREATE TABLE IF NOT EXISTS ${tableName} (
        ${cmd}
      );`)
      // per ora da errore la creo a mano
      // await sql`CREATE TABLE IF NOT EXISTS madonna (dio json)`
      await sql`CREATE TABLE IF NOT EXISTS ${sql(tableName)} (` + test + `)`
      return
    }
    let cmd = ''
    for (const fld of tableFieldsDef) {
      cmd += `"${fld.name}" ${fld.dataType}${fld.default ? ' ' + fld.default : ''},`
    }
    cmd = cmd.substring(0, cmd.length - 1)

    return createTable(cmd)/* sql`CREATE TABLE IF NOT EXISTS ${sql(tableName)} (
        ${cmd}
    );`; */
  }
  ...
  log(info, callback) {
    ...
    if (this.silent !== true) {
      const { defaultMeta, sql, tableFields, tableFieldsDef, tableName } = this;
      ...
      for (const extraField of tableFieldsDef) {
        if (Object.keys(log).indexOf(extraField.name) === -1) {
          if (extraField.dataType === 'json')
            log[extraField.name] = JSON.stringify(info[extraField.name])
          else
            log[extraField.name] = info[extraField.name]
        }
      }
